...
const SYSTEM_PROMPT = `You are a professional, polite, and conversational portfolio assistant for Yashwanth Reddy Reddygari. Your ONLY purpose is to answer questions about Yashwanth based on the information below. If someone asks about anything unrelated to Yashwanth, politely redirect them back to his professional profile.

IMPORTANT INSTRUCTIONS FOR YOUR RESPONSES:
1. Be Clean and Concise: Do NOT info-dump. Keep your answers brief, to the point, and highly readable.
2. Be Professional and Polite: Maintain a welcoming, respectful, and clear tone. 
3. Summarize First: If a user asks a broad question (e.g., "What are his projects?" or "Tell me about his experience"), highlight 2 or 3 of the most relevant points, and then politely ask if they would like to hear more.
4. Formatting: Use bullet points for lists to ensure clarity. Use plain text. Avoid overly long paragraphs.
...
FINAL REMINDER: Do not overwhelm the user with text. Keep responses light, professional, and conversational. If there is a lot of information to share, provide a high-level summary and ask, "Would you like me to go into more detail about any of these?`;
...
const FALLBACKS: { pattern: RegExp; response: string }[] = [
   {
      pattern: /yashwanth|who are you|introduce yourself|tell me about him/i,
...
→ Open to full-time SDE roles, graduating 2026`,
   ...
   {
      pattern: /his experience|his internship|his work|isro|dlrl|drdo|nrsc/i,
...
7. Fake Product Identification — QR-based counterfeit detection`,
   ...
   {
      pattern: /his skills|his tech|his stack|what.*he.*know|his tools/i,
...
→ Class X — Bhashyam High School (2019–2020), 10.0 GPA`,
   ...
   {
      pattern: /his hobbies|his sports|badminton|cricket|his personal|outside.*work/i,
...
function getFallbackResponse(message: string): string {
   ...
   return `I can only answer questions about Yashwanth Reddy Reddygari — his projects, experience, skills, and background. Try asking something like "What are his projects?" or "Tell me about his internships."`;
}
...
export async function POST(req: NextRequest) {
   ...
   if (apiKeys.length === 0) {
      console.error("[Chat] No API keys found in environment variables");
      return NextResponse.json({ response: getFallbackResponse(message) });
   }

   console.log(`[Chat] Found ${apiKeys.length} API key(s), history: ${history.length} msgs`);

   // Build multi-turn contents: past messages + current user message
   const contents = [
   ...
   for (let keyIdx = 0; keyIdx < apiKeys.length; keyIdx++) {
      ...
      for (let attempt = 0; attempt < 2; attempt++) {
         try {
            console.log(`[Chat] key[${keyIdx}] attempt ${attempt + 1}`);
            ...
            console.log(`[Chat] Success with key[${keyIdx}]`);
            return NextResponse.json({ response: text });
         } catch (err: any) {
            console.error(`[Chat] key[${keyIdx}] attempt ${attempt + 1} failed:`, {
               status: err?.status,
               message: err?.message,
            });
